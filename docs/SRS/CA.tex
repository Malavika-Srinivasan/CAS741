...
Regression and Interpolation. \wss{You don't need capitals here.}
...
\wss{It is good to have text between section headings.  You should put a mini
  roadmap here explaining the contents of this section.}
...
through a series of points. \wss{best fit is for regressions.  Interpolation is
...
\wss{Why aren't there output variabilities?  You could have the output to a file
  or to the screen or to memory.  You could have output variabilities to include
  certain data in the output, or not.  For instance, the residual could be part
  of the output.}
...
\item[R\refstepcounter{reqnum}\thereqnum \label{R_ICalculate}:] Compute best fit
  parameters. \wss{How?  You should reference the IMs that tell you how to do this.}

\item[R\refstepcounter{reqnum}\thereqnum \label{R_IOutput}:] Display the plot
  with data(\rref{R_IInputs}) and best fit result from
  \rref{R_ICalculate}. \wss{These are actually part of output variabilities.
    The calling program might not always need these services.}
...
\begin{itemize}
	\item \famname{} should be verifiable, which means the results should be
          comparable to Matlab. \wss{ambiguous.  What results will be compared?
            You could point to your verification plan here.}
\end{itemize}

\subsubsection{Maintainability and Support Requirements}
\begin{itemize}
	\item \famname{} should be able to accommodate new fitting methods for
          linear problems. \wss{What new fitting methods are possible?  How long
            will it take to add them?}
\end{itemize}	
...
\wss{Either say ``not applicable'' or remove this sections.}
