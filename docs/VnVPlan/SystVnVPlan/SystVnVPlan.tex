...
\title{CFS: System Verification and Validation Plan \wss{good to see the library
  name in the title}} 
...
\wss{if you don't have any figures, you can comment out this heading}
...
\wss{good to have a link to your repo.}
...
\famname{} \wss{I recently modified the blank project template to put the
  ...
  between all your files.  You might want to do the same.}and obtain confidence
...
\wss{You still don't have lines of text that are 80 characters long (separated
  by hard returns) in the tex file.}
...
\paragraph{Correctness } \wss{do you mean for this to be a subsection (not a
  paragraph), like the other subsections?}  The degree to which a product or
...
explained in the CA document for Program \famname{}. \wss{Reference your SRS.}
...
my classmates Ms. Jennifer Garner and Mr. Brooks MacLachlan. \wss{Good to list
  the specific people.  You should also split this up between the MG and the MIS.}
...
Output: Error message (``Please enter atleast \wss{proof read} 2 points'')
...
\wss{How did you come up with the output?  How does the reader verify this?  A
  similar comment applies elsewhere in your documentation.}
...
\item{\textbf{T6: $I^{st}$ test case for Lagrange's interpolation} \\} \wss{You
    ...
    second test case cover something different from the first?}  
Control:
  Automatic
...
How test will be performed: Pyunit \wss{You repeat this same test case several
...
\item{\textbf{T11: ${II}^{nd}$ test case for hermite \wss{Hermite} cubic interpolation} \\}
...
$Admissible\_error$ is available in appendix section.  \wss{This isn't a good
symbol.  A constant like this should be in all caps and it won't space out
properly.  Use something like $\mathit{ADMISS\_ERR}$.}\\
				
Output/Result: Pass/Fail \wss{The output should be the expected result.  Since
  you are summarizing multiple test cases at once, you could think of your
  output as a table of all of the relative errors, or you could report the
  maximum relative error (infinity norm.)}
...
How test will be performed: Manual \wss{I like this test case experiment.}
...
\wss{You should write numbers properly $1 \times 10^{-1}$}
