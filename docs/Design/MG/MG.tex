...
\item[\refstepcounter{acnum} \actheacnum \label{acInput}:] The format of the
  initial input data.
\item[\refstepcounter{acnum} \actheacnum \label{acSmoothing}:] The method used
  to calculate best fit may be smoothing instead of interpolation and
  regression.
...
\item[\refstepcounter{ucnum} \uctheucnum \label{ucInput}:] There will always be
  a source of input data external to the software. \wss{I think we should remove
    this UC.  It still doesn't really make sense to me.  I have removed it from
    the blank project template.}
\item[\refstepcounter{ucnum} \uctheucnum \label{ucNonLinear}:] If \famname{} is
  used with non linear systems, then the results from the software is no longer
  valid.
\end{description}
...
\item[Secrets:]This is the user program which is abstracted as a module which
  uses the best fit module to use the appropriate service. \wss{This isn't
...
\begin{description}
	...
	\item[Services:] Provides the fit coefficients. \wss{I removed the word
            best.  With interpolation there is only one fit.}
	\item[Implemented By:] \famname{}
\end{description}
...
\wss{This uses hierarchy is different from the one that I commented on in the
  issue tracker.  It seems the main difference is the ``best fit'' module.  For
  both diagrams it would make sense to draw an arrow from the top to input and
  output.  The user program will use the input and output modules.  This uses
  relation is covered by transitive closure, but I like it better when it is
  explicit.}
